...
func (plugin *glusterfsPlugin) NewMounter(spec *volume.Spec, pod *api.Pod, _ volume.VolumeOptions) (volume.Mounter, error) {
	...
	//ns := pod.Namespace
	//ns := ep_namespace
	ns := defaultendpointns
	ep, err := plugin.host.GetKubeClient().Core().Endpoints(ns).Get(ep_name)
	if err != nil && len(ep_namespace) == 0 {
		glog.Errorf("glusterfs: failed to get endpoints %s[%v]", ep_name, err)
		return nil, err
	}
	glog.V(2).Infof("glusterfs: endpoints %v ep_namespace %v", ep, ep_namespace)
	return plugin.newMounterInternal(spec, ep, pod, plugin.host.GetMounter(), exec.New(), ep_namespace)
}
...
func (b *glusterfsMounter) setUpAtInternal(dir string) error {
	...
	if b.hosts != nil || b.endpointsns != "" {
		...
		if b.hosts.Subsets != nil {
			...
			//addrlist = b.servers
			glog.V(1).Infof("glusterfs: both endpoint and endpointsNameSpace are nil")
			return fmt.Errorf("glusterfs: both endpoint and endpointsNameSpace are nil")
		}
...
func (d *glusterfsVolumeDeleter) Delete() error {
	...
	epsername := dstrings.Replace(volumetodel, "_","-", -1)
	glog.V(2).Infof("glusterfs: volume %s deleted successfully", volumetodel)
	err = d.plugin.host.GetKubeClient().Core().Endpoints(defaultendpointns).Delete(epsername,nil)
	if err != nil {
		glog.Errorf("glusterfs: failed to delete endpoint %s  error : %v", epsername, err)
	}
	glog.V(1).Infof("glusterfs: endpoint %s deleted successfully", epsername)
	err = d.plugin.host.GetKubeClient().Core().Services(defaultendpointns).Delete(epsername,nil)
	if err != nil {
			glog.Errorf("glusterfs: failed to delete service %s error %v", epsername, err)
	}
	glog.V(1).Infof("glusterfs: service %s deleted successfully", epsername)
	return nil
}
...
func (p *glusterfsVolumeProvisioner) CreateVolume() (r *api.GlusterfsVolumeSource, size int, err error) {
	...
	depsertoc := dstrings.Replace(volume.Name, "_","-", -1)
	dep := &api.Endpoints{
			//TypeMeta:unversioned.TypeMeta{Kind:"Endpoints", APIVersion:"v1"},
			ObjectMeta: api.ObjectMeta{Name: depsertoc, Namespace:defaultendpointns},
	...
	dser := &api.Service{
		//TypeMeta:unversioned.TypeMeta{Kind:"Service", APIVersion:"v1"},
			ObjectMeta: api.ObjectMeta{Namespace: defaultendpointns, Name: depsertoc},
			Spec: api.ServiceSpec{
				Ports: []api.ServicePort{
					{Protocol: "TCP", Port: 1}}}}
	glog.V(1).Infof("glusterfs: dynamic endpoint :%#v and service:%#v", dep, dser)
	_, err = p.plugin.host.GetKubeClient().Core().Endpoints(defaultendpointns).Create(dep)
	if err != nil {
		glog.Errorf("glusterfs: failed to create endpoint %s", err)
		return nil, 0, fmt.Errorf("error creating endpoint %v", err)
	}
	_, err = p.plugin.host.GetKubeClient().Core().Services(defaultendpointns).Create(dser)
	if err != nil {
			glog.Errorf("glusterfs: failed to create service %s", err)
			return nil, 0, fmt.Errorf("error creating service %v", err)
	}
	return &api.GlusterfsVolumeSource{
		...
		EndpointsNameSpace: defaultendpointns,
		//Path:          "myVol1",
		ReadOnly:      false,
	}, sz, nil
}
